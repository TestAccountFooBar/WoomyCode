...
!function () {
    ...
    const _imageCache = (function loadImages() {
        ...
        for (let image of [
            ...
            ["//media.discordapp.net/attachments/901620334363545600/1106696104692690954/Untitled.png", "jekyll_badge"],
    ...
    function initCarrierButtons(stuff) {
        ...
        const table = {
            ...
            "fighter": ["fighter", "Fighter Squadron", function click() {
                _socket.talk("cv", 0, "fighter");
            }]
        };
        stuff.forEach(thing => {
            if (table[thing] != null) {
    ...
    _util._getSpecialNameInfoById = id => [
        ...
        ["#876D58", "#644E3A", `'Share Tech Mono', monospace`, 1.4, "jekyll_badge"],
        ...
        return function ag(port) {
            ...
            socket.onmessage = function (message, parent) {
                ...
                switch (packet) {
                    ...
                    case "am":
                        //_anims[m[0]] = [];
                        //for (let i = 1; i < m.length; i++) _anims[m[0]].push(m[i]);
    ...
    let drawEntity = function () {
        ...
        function handleAnimation(animInfo) {
            if (animInfo.id) _socket.talk("am", animInfo.id);
            if (_anims[animInfo.id] != undefined) {
                switch (_anims[animInfo.id][0]) {
                    case 0: // Surge
                        ...
                        break;
                    case 1: // Bloodbath
                        let bloodbathTimer = _anims[animInfo.id][1];

                        if (animInfo.propIndex == 1 || animInfo.propIndex == 2) setColors(animInfo.context, mixColors(animInfo.propColor, '#FF0000', bloodbathTimer / 50));
                        if (animInfo.propIndex == 3) animInfo.props[animInfo.propIndex].size = 4 * bloodbathTimer / 50;
                        break;
                    default:
                        break;
                }
